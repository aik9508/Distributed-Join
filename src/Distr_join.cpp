...
vector<vector<vector<int>*> > distribute_vector(Relation &rel, int const numtasks, int* permu)
{
...
void get_scatter_v (Relation& rel, vector<int> &scatter_v, int const numtasks, int const arity, int*permu, int *displ, int *send_counts){
...
int main(int argc, char **argv)
{
    ...
    int numtasks, taskid;
    MPI_Status status;
    int *permu1, *permu2, *send_displ1, *send_displ2, *send_counts1, *send_counts2;
    ...
    if (taskid == root)
    {
        ...
        get_scatter_v(r1,scatter_v1,numtasks,arity1,permu1,send_displ1,send_counts1);
        get_scatter_v(r2,scatter_v2,numtasks,arity2,permu2,send_displ2,send_counts2);
    ...
    if(taskid==root) printf("[%6.4f]scatter data...\n",(double)(clock()-begin)/CLOCKS_PER_SEC);
    MPI_Scatter(send_counts1,1,MPI_INT,&loc_count1,1,MPI_INT,root,MPI_COMM_WORLD);
    MPI_Scatter(send_counts2,1,MPI_INT,&loc_count2,1,MPI_INT,root,MPI_COMM_WORLD);
    int loc_data1[loc_count1];
    int loc_data2[loc_count2];
    MPI_Scatterv(scatter_v1.data(),send_counts1,send_displ1,MPI_INT,loc_data1,loc_count1,MPI_INT,root,MPI_COMM_WORLD);
    MPI_Scatterv(scatter_v2.data(),send_counts2,send_displ2,MPI_INT,loc_data2,loc_count2,MPI_INT,root,MPI_COMM_WORLD);
    ...
    MPI_Gather(&join_count,1,MPI_INT,recv_counts,1,MPI_INT,root,MPI_COMM_WORLD);
    int* array_gather;
    int* displs;
    int total_count = 0;
    if(taskid==root){
    ...
    MPI_Gatherv(v_joined.data(),join_count,MPI_INT, array_gather, recv_counts,displs,MPI_INT, root, MPI_COMM_WORLD);
    if(taskid == root){
        printf("[%6.4f]vector gather finished...\n",(double)(clock()-begin)/CLOCKS_PER_SEC);
        vector<vector<int> > v_gather;
        reconstruct_loc_vector(v_gather,array_gather,total_count,(arity1+arity2-nj));
